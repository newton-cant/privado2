...
class WAConnection extends events_1.EventEmitter {
    constructor() {
        super(...arguments);
        /** A versão do WhatsApp Web que estamos informando aos servidores que estamos */
        this.version = [3, 3234, 9];
        /** O navegador que estamos informando aos servidores da Web do WhatsApp que estamos */
        this.browserDescription = Utils.Browsers.baileys('By: Tobi');
        /** As solicitações devem ser enfileiradas quando a conexão for interrompida; se 0, então um erro será lançado */
        ...
        this.connectOptions = {
            ...
            phoneResponseTime: 900,
            maxQueryResponseTime: 10000,
            alwaysUseTakeover: true,
            queryChatsTillReceived: true,
            logQR: true
        };
        /** Quando reconectar automaticamente */
        this.autoReconnect = Constants_1.ReconnectMode.onConnectionLost;
        /** Se o telefone está conectado */
        this.phoneConnected = false;
        /** chave para usar para solicitar bate-papos */
        ...
        this.referenceDate = new Date(); // usado para gerar tags
        this.lastSeen = null; // último manter vivo recebido
        ...
        // tempo limite para saber quando terminamos de receber mensagens
        this.messagesDebounceTimeout = Utils.debouncedTimeout(2000);
        // ping canto até recebido
        this.chatsDebounceTimeout = Utils.debouncedTimeout(10000);
        /**
         * Faz uma solicitação de busca com a configuração da conexão
         */
    ...
    /**
     * Conecte-se ao WhatsAppWeb
     * @param options as opções de conexão
     */
    async connect() {
        return null;
    }
    ...
    /** Você pode fazer o login no WA sem escanear o QR */
    canLogin() {
        var _a, _b;
        return !!((_a = this.authInfo) === null || _a === void 0 ? void 0 : _a.encKey) && !!((_b = this.authInfo) === null || _b === void 0 ? void 0 : _b.macKey);
    }
    /** Limpe as informações de autenticação para que uma nova conexão possa ser criada */
    clearAuthInfo() {
        this.authInfo = null;
        return this;
    }
     ...
     * @param authInfo as credenciais de autenticação ou caminho de arquivo para credenciais de autenticação
     */
    loadAuthInfo(authInfo) {
        if (!authInfo)
            throw new Error('dado authInfo é nulo');
        ...
        if ('clientID' in authInfo) {
            ...
            this.authInfo = {
                ...
                macKey: Buffer.from(secretBundle.macKey, 'base64'), // decodificar de base64
            };
        }
        return this;
    }
    /**
     * Aguarde até que uma mensagem com uma determinada tag seja recebida
     * @param marca a marca da mensagem a aguardar
     * @param json query que foi enviada
     * @param timeoutMs timeout após o qual a promessa será rejeitada
     */
    async waitForMessage(tag, requiresPhoneConnection, timeoutMs) {
        ...
        try {
            const result = await Utils.promiseTimeout(timeoutMs, (resolve, reject) => {
                ...
                this.on('ws-close', onErr); // se o soquete fechar, você nunca receberá a mensagem
            });
            return result;
        }
        finally {
    ...
    /** Função genérica para ação, definir consultas */
    ...
    /**
     * Consultar algo nos servidores WhatsApp
     * @param json a própria consulta
     * @param binaryTag as tags a serem anexadas se a consulta for enviada codificada em binário
     * @param timeoutMs 
     */
    async query(q) {
        ...
        const listener = ({ connected }) => {
            if (connected) {
                timeout = setTimeout(() => {
                    ...
                    cancel({ reason: 'Não esperando uma resposta', status: 422 });
    ...
    /** o intervalo é iniciado quando uma consulta demora muito para responder */
    startPhoneCheckInterval() {
        this.phoneCheckListeners += 1;
        if (!this.phoneCheckInterval) {
            // se já faz muito tempo e não temos resposta de WA, envie um ping
            this.phoneCheckInterval = setInterval(() => {
                if (!this.conn)
                    return; // se desconectado, então não faça nada
                this.logger.info('verificando a conexão do telefone...');
                this.sendAdminTest();
    ...
    /** verifica a conexão do telefone */
    async sendAdminTest() {
        return this.sendJSON(['admin', 'test']);
    }
    /**
     * Envie uma mensagem codificada em binário
     * @param json a mensagem a codificar e enviar
     * @param marca as marcas binárias para informar ao WhatsApp do que se trata a mensagem
     * @param marca a marca a ser anexada a t
     */
    async sendBinary(json, tags, tag = null, longTag = false) {
        const binary = this.encoder.write(json); // codificar o JSON para o formato binário WhatsApp
        let buff = Utils.aesEncrypt(binary, this.authInfo.encKey); // criptografá-lo usando AES e nossa encKey
        const sign = Utils.hmacSign(buff, this.authInfo.macKey); // assine a mensagem usando HMAC e nosso macKey
        tag = tag || this.generateMessageTag(longTag);
        ...
        buff = Buffer.concat([
            ...
            Buffer.from(tags),
            sign,
            buff, // o buffer criptografado real
        ]);
        await this.send(buff); // mande-o fora
        return tag;
    }
    /**
     * Envie uma mensagem JSON simples para os servidores WhatsApp
     * @param json a mensagem a enviar
     * @param marca a marca a ser anexada à mensagem
     * @returns a tag da mensagem
     */
    ...
    /** Envie alguma mensagem para os servidores WhatsApp */
    async send(m) {
        this.conn.send(m);
    }
    ...
    /**
     * Desconecte-se do telefone. Suas credenciais de autenticação tornam-se inválidas após o envio de uma solicitação de desconexão.
     * @ver close () se você quiser apenas encerrar a conexão
     */
    async logout() {
        this.authInfo = null;
        if (this.state === 'open') {
            //throw new Error("Você nem mesmo está conectado, você não pode sair")
    ...
    /** Feche a conexão com o WhatsApp Web */
    close() {
        this.closeInternal(Constants_1.DisconnectReason.intentional);
    }
    closeInternal(reason, isReconnecting = false) {
        this.logger.info(`closed connection, reason ${reason}${isReconnecting ? ', reconectando em alguns segundos...' : ''}`);
        this.state = 'close';
        this.phoneConnected = false;
    ...
    generateMessageTag(longTag = false) {
        ...
        this.msgCount += 1; // incrementar a contagem de mensagens, torna o campo 'época' ao enviar mensagens binárias
        return tag;
    }
}
